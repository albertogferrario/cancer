...
pub fn derive_inertia_props_impl(input: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        impl #impl_generics ::cancer::serde::Serialize for #name #ty_generics #where_clause {
            fn serialize<S>(&self, serializer: S) -> ::core::result::Result<S::Ok, S::Error>
            where
                S: ::cancer::serde::Serializer,
            {
                use ::cancer::serde::ser::SerializeStruct;
...
pub fn inertia_response_impl(input: TokenStream) -> TokenStream {
    ...
    let props_expr = match &input.props {
        PropsKind::Typed(expr) => {
            ...
            quote! {
                ::cancer::serde_json::to_value(&#expr)
    ...
    let expanded = if let Some(config) = input.config {
        ...
        quote! {{
            ...
            let url = ::cancer::InertiaContext::current_path();
            let response = ::cancer::InertiaResponse::new(#component_lit, props, url)
                .with_config(#config_expr);

            if ::cancer::InertiaContext::is_inertia_request() {
