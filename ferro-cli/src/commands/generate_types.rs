...
pub fn generate_typescript_with_imports(
    ...
    // Find types to import from shared.ts
    let mut imports_needed = Vec::new();
    if let Some(path) = project_path {
        let shared_types = parse_shared_types(path);
...
pub fn generate_types_to_file(project_path: &Path, output_path: &Path) -> Result<usize, String> {
    ...
    // Use the version with imports, passing the project path
    let typescript = generate_typescript_with_imports(&structs, Some(project_path));
