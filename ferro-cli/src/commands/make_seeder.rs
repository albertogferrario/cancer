...
pub fn run(name: String) {
    ...
    // Append "Seeder" suffix if not already present
    ...
    if !seeders_dir.exists() {
        if let Err(e) = fs::create_dir_all(seeders_dir) {
            eprintln!(
                "{} Failed to create seeders directory: {}",
                style("Error:").red().bold(),
                e
            );
            std::process::exit(1);
        }
        println!("{} Created src/seeders/", style("✓").green());

        // Create mod.rs
        let mod_content = templates::seeders_mod();
        ...
        println!("{} Created src/seeders/mod.rs", style("✓").green());
    }

    // Check if seeder file already exists
    if seeder_file.exists() {
        eprintln!(
            "{} Seeder '{}' already exists at {}",
    ...
    let seeder_content = templates::seeder_template(&file_name, &struct_name);

    // Write seeder file
    if let Err(e) = fs::write(&seeder_file, seeder_content) {
        eprintln!(
            "{} Failed to write seeder file: {}",
            style("Error:").red().bold(),
            e
        );
        std::process::exit(1);
    }
    println!("{} Created {}", style("✓").green(), seeder_file.display());
    ...
    println!("{} Updated src/seeders/mod.rs", style("✓").green());
    ...
    println!("Next steps:");
    println!(
        "  {} Implement your seeder logic in {}",
        style("1.").dim(),
        seeder_file.display()
    );
    println!();
    println!(
        "  {} Register the seeder in src/seeders/mod.rs:",
        style("2.").dim()
    );
    println!(
        "     {}",
    ...
    println!();
    println!("  {} Run the seeders:", style("3.").dim());
    println!(
        "     ./target/debug/app db:seed        {} Run all seeders",
        style("#").dim()
    );
    println!(
        "     ./target/debug/app db:seed --class {} {} Run specific seeder",
        struct_name,
        style("#").dim()
    );
    println!();
