...
pub fn run(name: String) {
    ...
    // Append "Event" suffix if not already present
    let struct_name = if struct_name.ends_with("Event") {
        struct_name
    } else {
        format!("{}Event", struct_name)
    };
    ...
    if !events_dir.exists() {
        ...
        println!("{} Created src/events/", style("✓").green());

        // Create mod.rs
        let mod_content = templates::events_mod();
        ...
        println!("{} Created src/events/mod.rs", style("✓").green());
    }

    // Check if event file already exists
    if event_file.exists() {
        eprintln!(
            "{} Event '{}' already exists at {}",
    ...
    if mod_file.exists() {
        ...
        if mod_content.contains(&mod_decl) || mod_content.contains(&pub_mod_decl) {
            eprintln!(
                "{} Module '{}' is already declared in src/events/mod.rs",
                style("Info:").yellow().bold(),
                file_name
            );
            std::process::exit(0);
    ...
    let event_content = templates::event_template(&file_name, &struct_name);

    // Write event file
    if let Err(e) = fs::write(&event_file, event_content) {
        eprintln!(
    ...
    println!("{} Created {}", style("✓").green(), event_file.display());
    ...
    println!("{} Updated src/events/mod.rs", style("✓").green());
    ...
    println!("Next steps:");
    println!(
        "  {} Add event data fields in {}",
        style("1.").dim(),
        event_file.display()
    );
    println!();
    println!(
        "  {} Add the events module to src/lib.rs or src/main.rs:",
        style("2.").dim()
    );
