...
pub fn handler_impl(_attr: TokenStream, input: TokenStream) -> TokenStream {
    ...
    if params.is_empty() {
        let output = quote! {
            #(#fn_attrs)*
            #fn_vis #async_token fn #fn_name #fn_generics(_: cancer::Request) #fn_output {
                #fn_block
            }
        };
        return output.into();
    ...
    let output = if has_request_param {
        ...
        quote! {
            #(#fn_attrs)*
            #fn_vis #async_token fn #fn_name #fn_generics(__cancer_req: cancer::Request) #fn_output {
            ...
            quote! {
                let #pat: #ty = {
                    let __value = __cancer_params.get(#param_name)
                        .ok_or_else(|| cancer::FrameworkError::param(#param_name))?;
                    <#ty as cancer::FromParam>::from_param(__value)?
                };
            }
        }
        ParamKind::Model => {
            ...
            quote! {
                let #pat: #ty = {
                    let __value = __cancer_params.get(#param_name)
                        .ok_or_else(|| cancer::FrameworkError::param(#param_name))?;
                    <#ty as cancer::AutoRouteBinding>::from_route_param(__value).await?
                };
            }
        }
        ParamKind::FormRequest => {
            ...
            *has_consumer = true;
            quote! {
                let #pat: #ty = <#ty as cancer::FromRequest>::from_request(__cancer_req).await?;
            }
        }
    }
}
